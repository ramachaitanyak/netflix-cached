...
void
Cache::getKeyValues(int io_handle, NetflixCached::ParsedPayload& payload) {
...
Cache::setKeyValue(NetflixCached::ParsedPayload& payload) {
...
Cache::processRequest(const std::pair<int, std::string> request) {
  ...
  if (result.first == OpCode::ERROR) {
    ...
    NetflixCached::ParsedPayload payload = result.second.second;
